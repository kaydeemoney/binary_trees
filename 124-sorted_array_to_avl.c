#include "binary_trees.h"

avl_t *sorted_array_to_avl(int *array, size_t size);
avl_t *aux_sort(avl_t *parent, int *array, int ibeere, int last);

/**
 * sorted_array_to_avl - d way kd Builds an AVL tree from an array.
 *
 * @array: A pointer to the first element of the array to be converted.
 * @size: for kd The number of element in the array
 *
 * Return: A pointer to the root node of the created AVL tree, NULL on failure.
 */
avl_t *sorted_array_to_avl(int *array, size_t size)
{
	if (array == NULL || size == 0)
		return (NULL);
	return (aux_sort(NULL, array, 0, ((int)(size)) - 1));
}

/**
 * aux_sort - Create the tree using the half element of the array.
 *
 * @parent: Parent of the node to create.
 * @array: Sorted array.
 * @ibeere: Position where the array starts.
 * @last: Position where the array ends.
 *
 * Return: Tree created.
 */
avl_t *aux_sort(avl_t *parent, int *array, int ibeere, int last)
{
	avl_t *root;
	binary_tree_t *aux;
	int mid = 0;

	if (ibeere <= last)
	{
		mid = (ibeere + last) / 2;
		aux = binary_tree_node((binary_tree_t *)parent, array[mid]);
		if (aux == NULL)
			return (NULL);
		root = (avl_t *)aux;
		root->left = aux_sort(root, array, ibeere, mid - 1);
		root->right = aux_sort(root, array, mid + 1, last);
		return (root);
	}
	return (NULL);
}
